...
## 下载安装包（双击安装，最适合电脑小白）
不会敲命令也没关系：下载下来双击就装，不用开终端、不用翻墙。
- 📥 **下载页（自动识别系统）**：<https://huhetingadday-boop.github.io/coding-agent-go/>
- 🍎 **macOS（Apple 芯片 M1/M2/M3/M4）**：[国内镜像 .dmg](https://gh-proxy.com/https://github.com/huhetingadday-boop/coding-agent-go/releases/latest/download/coding-agent-go-macos.dmg) · [GitHub 直连](https://github.com/huhetingadday-boop/coding-agent-go/releases/latest/download/coding-agent-go-macos.dmg)
...
下载后双击运行，浏览器会自动打开安装界面，跟着走就行。首次打开如果提示「身份不明的开发者」(macOS) 或 SmartScreen 蓝框 (Windows)，见 [安装包打不开怎么办](#安装包打不开怎么办)。Intel 芯片的 Mac 请用下面的一行命令安装。安装包来自 [Releases](https://github.com/huhetingadday-boop/coding-agent-go/releases/latest)，每个都附 SHA-256 校验值；下载页和发布版要等第一个版本 tag 构建出来后才有内容。
...
- 把 **AI Coding Go** 从 .dmg 拖进「应用程序」。
- 在「应用程序」里 **右键点它 →「打开」**，再在弹窗里点「打开」。
