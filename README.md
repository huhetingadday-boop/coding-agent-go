...
> 没装 Python 也没关系：脚本会自动装（per-user，不弹 UAC）。如果命令行里偏好 cmd/批处理，也可以用：`$f="$env:TEMP\acgg.bat"; iwr https://gitee.com/huhetingadday-boop/coding-agent-go/raw/main/install-gui.bat -OutFile $f; cmd /c $f`
...
- **国内首选走 Gitee 镜像，备用走 ghproxy，都跟 `main` 分支走**。`gitee.com` 是国内域名，坏 DNS 也能解析，是国内最稳的下载源；连不上时再用主命令下面的 ghproxy 备用。想只跑打过 tag 的正式发布版，用 jsDelivr `@latest`：macOS/Linux `bash <(curl -fsSL https://cdn.jsdelivr.net/gh/huhetingadday-boop/coding-agent-go@latest/install-gui.sh)`，Windows `irm https://cdn.jsdelivr.net/gh/huhetingadday-boop/coding-agent-go@latest/install-gui.ps1 | iex`。
