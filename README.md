...
> 没装 Python 也没关系：脚本会自动装（per-user，不弹 UAC）。如果命令行里偏好 cmd/批处理，也可以用：`$f="$env:TEMP\acgg.bat"; iwr https://ghfast.top/https://raw.githubusercontent.com/huhetingadday-boop/coding-agent-go/main/install-gui.bat -OutFile $f; cmd /c $f`
...
- **主命令为国内网络优化，备用命令钉正式发布版**。macOS/Linux 和 Windows 主命令都走 ghproxy 直连 GitHub，绕开 jsdelivr 在国内偶发的 SSL 报错和限速；它们跟 `main` 分支走。想只跑打过 tag 的正式发布版，用主命令下面的 jsdelivr 备用命令。
