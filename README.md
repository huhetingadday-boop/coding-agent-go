...
```bash
bash <(curl -fsSL https://cdn.jsdelivr.net/gh/hhwyt/coding-agent-go@main/install-gui.sh)
```
### Windows PowerShell
打开「PowerShell」，粘贴这一行，回车（`irm | iex` 就是 PowerShell 版的 `curl | sh`）：
```powershell
irm https://cdn.jsdelivr.net/gh/hhwyt/coding-agent-go@main/install-gui.ps1 | iex
```
...
> 没装 Python 也没关系：脚本会自动装（per-user，不弹 UAC）。如果命令行里偏好 cmd/批处理，也可以用：`$f="$env:TEMP\acgg.bat"; iwr https://cdn.jsdelivr.net/gh/hhwyt/coding-agent-go@main/install-gui.bat -OutFile $f; cmd /c $f`
...
- 发送测试请求，确认模型真的能用
- Agent smoke test：通过 `gh api` 给 tikv/tikv 点 star，验证完整工具调用链路
